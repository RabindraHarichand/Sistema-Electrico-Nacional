...
export class CreateEnergySystemDto {
  constructor(
  ...
  static create(props: {
    ...
    if (links) {
      ...
      links.map((link: Link) => {
        ...
        //value
        if (!link.value) return ["Missing value"];
        ...
        //kwUnits
        if (!link.kwUnits) return ["Missing kwUnits"];
    ...
    return [
      undefined,
      new CreateEnergySystemDto(group, type, name, location, links),
    ];
  }
}
